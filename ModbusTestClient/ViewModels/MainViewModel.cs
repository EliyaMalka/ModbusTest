...
using ModbusTestClient.Helpers;
using ModbusTestClient.Models;
using ModbusTestClient.Services;

namespace ModbusTestClient.ViewModels
{
    public class MainViewModel : BaseViewModel, IDisposable
    {
        ...
        public string ReadRegStartAddress
        {
            get => _readRegStartAddress;
            set => SetProperty(ref _readRegStartAddress, value);
        }

        private string _readRegQuantity = "10";
        public string ReadRegQuantity
        {
            get => _readRegQuantity;
            set => SetProperty(ref _readRegQuantity, value);
        }

        private ObservableCollection<RegisterResult> _registerResults = new();
        public ObservableCollection<RegisterResult> RegisterResults
        {
            get => _registerResults;
            set => SetProperty(ref _registerResults, value);
        }

        // ========== Write Single Register (FC6) ==========
        private string _writeSingleAddress = "0";
        public string WriteSingleAddress
        {
            get => _writeSingleAddress;
            set => SetProperty(ref _writeSingleAddress, value);
        }

        private string _writeSingleValue = "0";
        public string WriteSingleValue
        {
            get => _writeSingleValue;
            set => SetProperty(ref _writeSingleValue, value);
        }

        // ========== Write Multiple Registers (FC16) ==========
        private string _writeMultiStartAddress = "0";
        public string WriteMultiStartAddress
        {
            get => _writeMultiStartAddress;
            set => SetProperty(ref _writeMultiStartAddress, value);
        }

        private string _writeMultiValues = "0,0,0";
        public string WriteMultiValues
        {
            get => _writeMultiValues;
            set => SetProperty(ref _writeMultiValues, value);
        }

        // ========== Read Coils (FC1) ==========
        private string _readCoilStartAddress = "0";
        public string ReadCoilStartAddress
        {
            get => _readCoilStartAddress;
            set => SetProperty(ref _readCoilStartAddress, value);
        }

        private string _readCoilQuantity = "10";
        public string ReadCoilQuantity
        {
            get => _readCoilQuantity;
            set => SetProperty(ref _readCoilQuantity, value);
        }

        private ObservableCollection<CoilResult> _coilResults = new();
        public ObservableCollection<CoilResult> CoilResults
        {
            get => _coilResults;
            set => SetProperty(ref _coilResults, value);
        }

        // ========== Write Single Coil (FC5) ==========
        private string _writeCoilAddress = "0";
        public string WriteCoilAddress
        {
            get => _writeCoilAddress;
            set => SetProperty(ref _writeCoilAddress, value);
        }

        private bool _writeCoilValue;
        public bool WriteCoilValue
        {
            get => _writeCoilValue;
            set => SetProperty(ref _writeCoilValue, value);
        }
        ...
        private async Task ConnectAsync()
        {
            try
            {
                ...
                if (!int.TryParse(TimeoutMs, out int timeout))
        ...
        private void Disconnect()
        {
            ...
            {
                AddLog($"Disconnect error: {ex.Message}", LogLevel.Error);
            }
        }

        private byte GetSlaveId()
        {
            return byte.TryParse(SlaveId, out byte sid) ? sid : (byte)1;
        }
        ...
        private async Task ReadHoldingRegistersAsync()
        {
            try
            {
                ...
                if (!ushort.TryParse(ReadRegQuantity, out ushort qty) || qty == 0)
                    throw new ArgumentException("Invalid quantity (must be 1-125)");
                ...
                {
                    RegisterResults.Add(new RegisterResult
                    {
                        Address = (ushort)(startAddr + i),
                        Value = values[i],
        ...
        private async Task WriteSingleRegisterAsync()
        {
            ...
            {
                IsBusy = true;
                if (!ushort.TryParse(WriteSingleAddress, out ushort addr))
        ...
        private async Task WriteMultipleRegistersAsync()
        {
            ...
            {
                IsBusy = true;
                if (!ushort.TryParse(WriteMultiStartAddress, out ushort startAddr))
                    throw new ArgumentException("Invalid start address");

                var parts = WriteMultiValues.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                if (parts.Length == 0)
                    throw new ArgumentException("No values provided. Enter comma-separated values.");
        ...
        private async Task ReadCoilsAsync()
        {
            ...
            {
                IsBusy = true;
                if (!ushort.TryParse(ReadCoilStartAddress, out ushort startAddr))
                    throw new ArgumentException("Invalid start address");
                if (!ushort.TryParse(ReadCoilQuantity, out ushort qty) || qty == 0)
                    throw new ArgumentException("Invalid quantity (must be 1-2000)");
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    CoilResults.Add(new CoilResult
                    {
                        Address = (ushort)(startAddr + i),
                        Value = values[i],
                        Timestamp = DateTime.Now
        ...
        private async Task WriteSingleCoilAsync()
        {
            ...
            {
                IsBusy = true;
                if (!ushort.TryParse(WriteCoilAddress, out ushort addr))
                    throw new ArgumentException("Invalid address");
        ...
        private async Task AutoRefreshTick()
        {
            ...
            try
            {
                // Re-read holding registers with current settings
                if (ushort.TryParse(ReadRegStartAddress, out ushort startAddr) &&
        ...
        private async Task RunBatchTestAsync()
        {
            try
            {
                ...
                try
                {
                    ...
                    passed++;
                    AddLog("Test 1: PASSED ✓", LogLevel.Success);
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 1: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    {
                        passed++;
                        AddLog($"Test 2: PASSED ✓ (read value = {result[0]})", LogLevel.Success);
                    }
                    else
                    {
                        failed++;
                        AddLog($"Test 2: FAILED ✗ (expected 12345, got {result[0]})", LogLevel.Error);
                    }
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 2: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    passed++;
                    AddLog("Test 3: PASSED ✓", LogLevel.Success);
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 3: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    {
                        passed++;
                        AddLog($"Test 4: PASSED ✓ (values = [{string.Join(", ", result)}])", LogLevel.Success);
                    }
                    else
                    {
                        failed++;
                        AddLog($"Test 4: FAILED ✗ (expected [100,200,300], got [{string.Join(", ", result)}])", LogLevel.Error);
                    }
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 4: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    passed++;
                    AddLog("Test 5: PASSED ✓", LogLevel.Success);
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 5: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    {
                        passed++;
                        AddLog("Test 6: PASSED ✓ (coil is ON)", LogLevel.Success);
                    }
                    else
                    {
                        failed++;
                        AddLog("Test 6: FAILED ✗ (expected ON, got OFF)", LogLevel.Error);
                    }
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 6: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    passed++;
                    AddLog("Test 7: PASSED ✓", LogLevel.Success);
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 7: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
                ...
                try
                {
                    ...
                    {
                        passed++;
                        AddLog("Test 8: PASSED ✓ (coil is OFF)", LogLevel.Success);
                    }
                    else
                    {
                        failed++;
                        AddLog("Test 8: FAILED ✗ (expected OFF, got ON)", LogLevel.Error);
                    }
                }
                catch (Exception ex)
                {
                    failed++;
                    AddLog($"Test 8: FAILED ✗ - {ex.Message}", LogLevel.Error);
                }
            ...
            {
                AddLog($"Batch test error: {ex.Message}", LogLevel.Error);
            }
            finally
            {
                IsBusy = false;
        ...
        private void ExportLog()
        {
            ...
            {
                AddLog($"Export error: {ex.Message}", LogLevel.Error);
            }
        }
